...
独立的 OpenCode Go 多账号额度看板，提供 Docker 镜像、Web 管理后台、加密 Key 存储和真实模型请求测试。不依赖 DeepSeek Harness、Cordis、Typert 或本机 OpenCode 客户端。
...
- API Key 使用 scrypt 派生密钥和 AES-256-GCM 加密后保存到 Docker 数据卷。
...
`npm run preview` 只监听 `127.0.0.1:57726`，并在 Git 忽略的 `.local-runtime` 中保存本地预览数据，不代替正式 Docker 部署。
