...
- 每个 Key 可设置持久化分组；后台支持按分组筛选，并按分组名称升序或降序排列。未填写分组的旧数据会显示在“未分组”区块。
- 每个 Key 资产节点都可以单独点击“刷新额度”，只强制查询该 Key，不会刷新其他账号；“测试模型”仍是发送真实模型请求的独立操作。
- Key 资产节点使用独立边框和间距清晰分组；支持逐项选择、全选当前筛选结果和批量删除，环境变量 Key 始终保持只读且不会被选中。
...
添加或编辑 Key 时可以填写分组名称（最多 60 个字符，不能包含换行）。分组保存在 Docker 加密文件或 Workers D1 中，不会因为刷新页面、重启容器、自动刷新或备份恢复而丢失。后台列表提供“全部分组”筛选和三种排序方式：默认顺序（按分组区块显示，组内保持原账号顺序）、分组升序、分组降序；未分组始终排在最后。环境变量注入的只读 Key 不属于任何分组。

升级已有 Workers 部署时，必须先备份 D1 并执行新的远程迁移（`0006_account_groups.sql`），再部署新代码；旧 D1 数据会自动兼容为“未分组”。
